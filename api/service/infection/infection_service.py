...
async def report_infection(survivor_id: int, report: ReportInfection, db: AsyncSession) -> dict:
    ...
    if reporter.infected:
        raise HTTPException(status_code=400, detail="Infected survivors cannot report others")
    ...
    if existing_report.first():
        raise HTTPException(
            status_code=400,
            detail="You have already reported this survivor"
        )
    ...
    try:
        await db.execute(
            infection_reports.insert().values(
                ...
                reported_id=survivor_id
            )
        )
        await db.commit()

        report_count = await db.execute(
            select(func.count()).select_from(infection_reports).where(
                infection_reports.c.reported_id == survivor_id
            )
        )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to report infection: {str(e)}") 