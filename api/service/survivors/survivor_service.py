...
from models import SurvivorCreate, Location, Survivor, Inventory
from fastapi import HTTPException
from typing import Optional, List
from service.inventory.inventory_service import add_inventory_items, get_survivor_inventory
...
async def create_survivor(survivor: SurvivorCreate, db: AsyncSession) -> Survivor:
    ...
    if existing_survivor:
        raise HTTPException(
            status_code=400,
            detail="A survivor with this name already exists"
        )
...
async def update_location(survivor_id: int, location: Location, db: AsyncSession) -> dict:
